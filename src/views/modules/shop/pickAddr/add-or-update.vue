...
const init = (id) => {
  ...
  if (dataForm.addrId) {
    http({
      ...
      .then(({ data }) => {
        dataForm.addr = data.data.addr
        dataForm.mobile = data.data.mobile
        dataForm.addrName = data.data.addrName
        dataForm.areaId = data.data.areaId
        dataForm.cityId = data.data.cityId
        dataForm.provinceId = data.data.provinceId
        listAreaByParentId(data.data.provinceId).then(({ data }) => {
          cityList.value = data.data
        })
        listAreaByParentId(data.data.cityId).then(({ data }) => {
...
const listAreaByParentId = (pid) => {
  if (!pid) pid = 0
  return http({
    url: http.adornUrl('/area/listByPid'),
    method: 'get',
