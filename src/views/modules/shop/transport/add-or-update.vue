...
const init = (id) => {
  ...
  if (dataForm.value.transportId) {
    http({
      // 获取运费模板数据
      url: http.adornUrl(`/transport/info/${dataForm.value.transportId}`),
      method: 'get'
    })
        .then(({data}) => {
          dataForm.value = data.data
          if (data.data.isFreeFee) {
            dataForm.value.transfees[0].status = 0
          } else {
            dataForm.value.transfees[0].status = 1
          }
          dataForm.value.hasFreeCondition = !!data.data.hasFreeCondition
        })
  }
}
defineExpose({init})
