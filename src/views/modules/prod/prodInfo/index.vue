...
const tags = ref([])// 分组信息
onMounted(() => {
  console.log(`[prodInfo index.vue] onMounted() prodId = `, useRoute().query.prodId)
...
const getDataList = () => {
  ...
  getCategoryList().then(() => {
    console.log(`[prodInfo index.vue] getDataList() prodId = `, dataForm.value.prodId)
    if (dataForm.value.prodId) {
      // 获取产品数据
      http({
        url: http.adornUrl(`/prod/sys/info`),
        method: 'get',
        params: {
        ...
        .then(({ data }) => {
          ...
          if (data.data.deliveryMode){
            dataForm.value.deliveryMode = JSON.parse(data.data.deliveryMode)
          }
          skuTagRef.value?.init(data.data.skuList)
          skuTableRef.value?.init()
          category.selected = idList(category.list, dataForm.value.categoryId, 'categoryId', 'children').reverse()
          dataForm.value.tagList = data.data.tagList
        })
    } else {
      nextTick(() => {
...
const getCategoryList = () => {
  console.log(`[prodInfo index.vue] getCategoryList() prodId = `, dataForm.value.prodId)
  return http({
    url: http.adornUrl('/category/listCategory'),
    method: 'get',
    params: http.adornParams()
  })
    .then(({ data }) => {
      category.list = treeDataTranslate(data.data, 'categoryId', 'parentId')
    })
}
/**
 * 选择分类改变事件
